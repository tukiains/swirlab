function [t,K,K2] = calc_direct_radiance(geo,wn,gasvec,cros,sol,p1,p2,p3,offset,L)
...
dens_mat  = cell2mat(struct2cell(geo.layer_dens));
...
% density * length
dens_los = reshape(repmat(geo.los_lens,1,ngas)',1,nlay*ngas)'.*dens_lay;
...
for n=1:ngas
    K(:,n) = -c * squeeze(cros(n,:,:))'*(dens_mat(n,:)'.*geo.los_lens) .*t .* base; % gases
end
